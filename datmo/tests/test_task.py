...
class TestTaskModule():
    ...
    def test_run(self):
        ...
        # 2) Test out option 2
        task_obj_0 = run(command="python script.py", home=self.temp_dir, mem_limit=test_mem_limit)
        ...
        # 3) Test out option 3
        task_obj_1 = run(
            command="python script.py", env=test_filepath, home=self.temp_dir, mem_limit=test_mem_limit)
        ...
        task_obj_2 = run(
            ...
            env=test_filepath,
            home=self.temp_dir, mem_limit=test_mem_limit)
        ...
        with open(test_filepath, "wb") as f:
            f.write(to_bytes("FROM datmo/xgboost:cpu"))
        task_obj_4 = run(command=["python", "script.py"], home=self.temp_dir, mem_limit=test_mem_limit)
