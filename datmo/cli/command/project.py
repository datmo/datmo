...
class ProjectCommand(BaseCommand):
    ...
    def status(self):
        ...
        if current_snapshot_id:
            self.cli_helper.echo("Current state of snapshot: %s" % current_snapshot_id)
            self.cli_helper.echo("")

        # Print out info for the latest snapshot autogenerated by datmo
        self.cli_helper.echo(
            "latest snapshot generated (run `datmo snapshot ls --all` for more info): "
        )
        if latest_snapshot_user_generated:
            self.cli_helper.echo(latest_snapshot_user_generated)
        elif latest_snapshot_auto_generated:
